...
- 모바일 퀴즈처럼 한 화면에서 판단이 끝나는 도구는 `100dvh` 안에 조작부를 모두 배치하고 전역 header/footer를 숨겨 문서 스크롤을 만들지 않는다. 카드는 수평 drag만 허용하며 완료 목록도 같은 화면 안에 맞춘다. 데스크톱에서는 430px 폭의 모바일 mockup surface를 중앙에 두고 모바일과 같은 정보 밀도를 유지한다.
