...
- 홈: 원본 확인 상태 tabs → 최신/연도 tabs → category tabs 순으로 큰 필터를 조합한다.
- 상세: 설명 → 현재 원본/커뮤니티 링크와 근거 → 바이럴 TOP 10 → 챌린지 참여 rail → 접힌 보조 타임라인 → 참여 → 다른 항목
- 관리자: 전역 상태 → tabs → 한 가지 작업 surface. 사전 탭 안에서는 검색/필터 → 일괄 도구 → 항목 카드 순서를 유지한다.
- 모바일 퀴즈처럼 한 화면에서 판단이 끝나는 도구는 `100dvh` 안에 조작부를 모두 배치하고 전역 header/footer를 숨겨 문서 스크롤을 만들지 않는다. 데스크톱에서는 일반 page shell로 복귀한다.
...
- 여러 영상은 iframe을 반복하지 않는다. 바이럴 대표 1개만 재생하고 나머지는 thumbnail selector, 참여 영상은 2행 link rail을 사용한다.
