...
- 대표 1개만 embed하는 바이럴 TOP 10과 2행 참여 영상 rail
- 관리자 사전 수정·삭제·검색·필터와 최대 100개 일괄 상태/카테고리/삭제
- 코리아 마이너 밈 kind/category, 선택형 설명·원본·타임라인과 복수 커뮤니티 링크
- oEmbed/Open Graph 썸네일 후보와 선택형 Gemma 설명 초안
- 인지도·상세 열람·설명 도움률을 분리하는 5카드 퀴즈와 관리자 로그
...
- 카드별 `know`/`dont_know`로 최초 인지도를 측정한다.
...
- 운영자는 유형별 인지도, 상세 수요, 설명 도움률과 원시 CSV를 확인한다.
