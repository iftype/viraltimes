...
export function registerIntakeRoutes(
  ...
  app.post("/api/v1/intake", async (request, reply) => {
    ...
    const isMemeRequest = category === "meme_request";
    const descriptionRequired = !isMemeRequest;
    if (
      title.length > 120 ||
      (!isMemeRequest && !title) ||
      ...
      return reply.code(400).send({ error: descriptionRequired ? "제목과 설명을 확인해 주세요." : "입력 내용을 확인해 주세요." });
    }
