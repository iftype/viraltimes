...
- 빌드: Vercel에서는 Next.js ISR을 사용하고 GitHub Pages와 Nginx 정적 호스팅에서는 static export를 사용한다.
...
- 상세 canonical URL은 `/memes/:slug`다. Vercel은 API 데이터를 5분 ISR로 받아 최초 HTML의 본문·metadata·Article JSON-LD를 만들고, static export는 빌드 시 공개 항목을 모두 미리 생성한다. `/sitemap.xml`도 웹 앱이 직접 생성해 외부 프록시 실패와 무관하게 제공한다. 제목 기반 한국어 검색어에는 원조·원본·최초·처음·시작·유래·뜻·밈·챌린지를 포함한다.
...
- `/quiz`는 공개 사전 전체에서 매번 무작위로 고른 5개 카드의 인지도를 익명 실행 단위로 측정한다. 결과 전에 고정 4항목 경험 체크리스트와 설정된 객관식 추가 설문을 표시하고, 실제 상세 페이지를 iframe modal로 열며 완료 화면에서 다른 참여자 통계와 상세·서비스 CTA를 제공한다.
