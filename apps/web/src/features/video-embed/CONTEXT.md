...
- 영상·피드 poster의 외부 썸네일이 차단·삭제·만료되면 깨진 이미지 대신 플랫폼 링크 placeholder를 표시한다.
- 퀴즈처럼 작은 surface에 embed만 넣는 경우 `compact` 모드로 metadata footer 없이 컨테이너를 채운다.
...
- 모바일 autoplay 정책을 위해 YouTube와 TikTok player는 항상 muted URL로 시작한다. YouTube는 공식 IFrame API의 `onReady`에서, TikTok은 공식 player `onPlayerReady`와 postMessage 재시도에서 재생·전역 소리 상태를 동기화한다. 브라우저가 YouTube autoplay를 명시적으로 차단하면 중앙에 재생 fallback을 표시한다.
