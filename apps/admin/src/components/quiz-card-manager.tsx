  ...
  return (
    <section className="mt-5 rounded-3xl border border-black/5 bg-white p-5 sm:p-7">
      ...
      <div className="mt-5 space-y-2">{drafts.map((card, index) => <article className="grid gap-2 rounded-2xl bg-[#f7f7f8] p-3 sm:grid-cols-[2.5rem_minmax(0,1fr)_10rem_auto] sm:items-center" key={card.id}>
        ...
        <label className="text-xs font-black text-black/45">사전 항목<select className="mt-1 w-full cursor-pointer rounded-xl border border-black/5 bg-white px-3 py-2.5 text-base font-bold" onChange={(event) => patch(index, { memeId: event.target.value })} value={card.memeId}><option value="">밈 선택</option>{published.map((meme) => <option disabled={drafts.some((candidate, candidateIndex) => candidateIndex !== index && candidate.memeId === meme.id)} key={meme.id} value={meme.id}>{meme.title}</option>)}</select></label>
