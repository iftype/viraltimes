...
export function QuizCardManager({ items, memes, onChange }: { items: QuizCardConfig[]; memes: AdminMeme[]; onChange: (items: QuizCardConfig[]) => void }) {
  ...
  return (
    <section className="mt-5 rounded-3xl border border-black/5 bg-white p-5 sm:p-7">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black text-[#fe2c55]">QUIZ LABEL</p><h2 className="mt-1 text-2xl font-black tracking-[-0.04em]">퀴즈 분야 라벨</h2><p className="mt-1 text-xs leading-5 text-black/40">퀴즈는 공개 사전 전체에서 매번 5개를 무작위 출제합니다. 필요한 항목에만 분야 라벨을 지정할 수 있습니다.</p></div><button className="flex cursor-pointer items-center gap-1.5 rounded-full bg-black px-4 py-2.5 text-xs font-black text-white disabled:opacity-35" disabled={drafts.length >= 5 || published.length === 0} onClick={() => setDrafts((current) => [...current, { id: `new-${Date.now()}`, memeId: published.find((meme) => !current.some((card) => card.memeId === meme.id))?.id ?? "", field: "마이너 밈", enabled: true, sortOrder: current.length, updatedAt: "" }])} type="button"><Plus className="size-3.5" />라벨 추가</button></div>
      ...
      </article>)}{drafts.length === 0 && <p className="rounded-2xl bg-[#f7f7f8] p-8 text-center text-sm font-bold text-black/35">별도 분야 라벨이 없습니다. 공개 사전 전체에서 5개가 무작위로 나옵니다.</p>}</div>
      <button className="mt-4 flex w-full cursor-pointer items-center justify-center gap-2 rounded-full bg-[#fe2c55] px-5 py-3 text-sm font-black text-white disabled:opacity-40" disabled={saving || drafts.some((card) => !card.memeId || !card.field.trim())} onClick={() => void save()} type="button">{saving ? <LoaderCircle className="size-4 animate-spin" /> : <Save className="size-4" />}{saving ? "저장 중" : "분야 라벨 저장"}</button>
