...
type Tab = "all" | Category | "final_review" | "dictionary" | "categories" | "quiz_logs" | "quiz_cards";
...
const tabs: { id: Tab; label: string; icon: typeof Bell }[] = [
  ...
  { id: "quiz_cards", label: "퀴즈 카드", icon: ListChecks },
...
export default function AdminPage() {
  ...
  const loadInbox = useCallback(async () => {
    ...
    setError("");
    try {
      const [inboxResponse, memeResponse, categoryResponse, quizLogsResponse, quizCardsResponse] = await Promise.all([
      ...
      if (!quizCardsResponse.ok) throw new Error(await readError(quizCardsResponse));
      ...
      const quizCardsData = (await quizCardsResponse.json()) as { items: QuizCardConfig[] };
      
      setItems(inboxData.items);
      setMemes(memeData.items);
      setCategories(categoryData.items);
      setQuizLogs(quizLogsData.items || []);
      setQuizCards(quizCardsData.items || []);
  ...
  const visibleItems = useMemo(() => {
    ...
    if (activeTab === "dictionary" || activeTab === "categories" || activeTab === "quiz_logs" || activeTab === "quiz_cards") return [];
  ...
  const countFor = (tab: Tab) => {
    ...
    if (tab === "quiz_cards") return quizCards.length;
  ...
  return (
    <main className="min-h-screen pb-16">
      ...
      <div className="admin-shell pt-8 sm:pt-12">
        ...
        {activeTab === "dictionary" ? (
        ...
        ) : activeTab === "quiz_cards" ? (
          <QuizCardManager key={quizCards.map((card) => `${card.id}:${card.updatedAt}`).join("|")} items={quizCards} memes={memes} onChange={setQuizCards} />
          ...
          {loading ? <div className="flex min-h-56 items-center justify-center rounded-3xl border border-black/5 bg-white"><LoaderCircle className="size-6 animate-spin text-black/25" /></div> : visibleItems.length === 0 ? <div className="flex min-h-64 flex-col items-center justify-center rounded-3xl border border-dashed border-black/10 bg-white px-6 text-center"><span className="flex size-12 items-center justify-center rounded-2xl bg-[#e8fffe] text-[#087b77]"><Check className="size-6" /></span><h2 className="mt-4 text-lg font-black">여기는 다 확인했어요</h2><p className="mt-1 text-sm leading-6 text-black/40">새로운 알림이 들어오면 이 탭에 바로 표시됩니다.</p></div> : visibleItems.map((item) => {
            ...
            return <article className="rounded-3xl border border-black/5 bg-white p-5 shadow-[0_8px_28px_rgba(0,0,0,0.04)] sm:p-6" key={item.id}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">{item.sourceUrl && <a className="inline-flex items-center gap-1.5 rounded-full bg-black/5 px-3 py-2 text-xs font-black" href={item.sourceUrl} rel="noreferrer" target="_blank">제보 영상<ExternalLink className="size-3.5" /></a>}{item.originUrl && <a className="inline-flex items-center gap-1.5 rounded-full bg-[#fff0f3] px-3 py-2 text-xs font-black text-[#d91d46]" href={item.originUrl} rel="noreferrer" target="_blank">제보자가 아는 원본<ExternalLink className="size-3.5" /></a>}</div>
